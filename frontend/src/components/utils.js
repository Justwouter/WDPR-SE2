...
export const getUur = vanDate => {
  let s =
    (new Date(vanDate).getMinutes() < 10 ? '0' : '') +
    new Date(vanDate).getMinutes()
  return (
    (new Date(vanDate).getHours() < 10 ? '0' : '') +
    new Date(vanDate).getHours() +
    ':' +
    s
  )
}
...
export async function checkElevationAPI (jwtToken) {
  let jwtRAW = jwtToken
