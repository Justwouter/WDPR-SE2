...
if ($args.Contains("-r")) {
    ...
    foreach ($folder in $folderlocations) {
        ...
        for ($i = 0; $i -lt $files.Count; $i++) {
            ((Get-Content -path $files[$i].FullName -Raw) -replace 'https://api.theaterlaak.site', 'http://api.localhost' -replace 'https://theaterlaak.site', 'http://frontend.localhost' -replace 'https://test.theaterlaak.site', 'http://test.localhost') | Set-Content -Path $files[$i].FullName -NoNewline
        }
